...
OUTPUT_DIR = Path("problem3_outputs_final")
...
# ==========================================
# 4. 最終調校的 QNN 架構（串行 Data Reuploading 版本）
# ==========================================
n_qubits = 6  # 增加到位元 6 個，能攜帶更多 CNN 特徵資訊
n_layers = 2  # 用作 serial data reuploading 的重複次數
dev = qml.device("default.qubit", wires=n_qubits)

@qml.qnode(dev, interface="torch", diff_method="backprop")
def quantum_circuit(inputs, weights):
    # Serial data reuploading:
    # Encode -> Variational -> Encode again -> Variational
    for layer in range(n_layers):
        # Re-upload the same reduced feature vector in every layer
        for i in range(n_qubits):
            qml.RY(inputs[i], wires=i)

        # Variational layer for this stage
...
print("\n--- 訓練串行 Data Reuploading 量子模型 (CNN+QNN) ---")
